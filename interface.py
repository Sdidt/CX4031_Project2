...
""" This is for Navigation Bar - Datasets Display """
...
""" This is a Modal for SQL Query example display """
...
""" This is the Modal handling for Navigation Bar, Error Message, Loading of the Page """
modal = html.Div([
        dbc.Modal([
            dbc.ModalHeader(dbc.ModalTitle("About")),
            dbc.ModalBody("This is a CZ4032 Database System Principles Project."),],
            id="modal-about", size="lg", is_open=False,
        ),
        dbc.Modal([
        ...
        dbc.Modal([
            ...
            ], backdrop="static", id="modal-startup", size="lg", is_open=True,
        ),
        dbc.Modal([
                dbc.ModalHeader(dbc.ModalTitle("Database Details")),
                dbc.ModalBody("Database details successfully submitted"),
            ], id="modal-database", size="lg", is_open=False,
        ),
])

""" This is the Navigation bar UI """
...
sql_query_div = html.Div(children=[
    ...
    dbc.CardBody([
        dbc.Popover(
            "Example : SELECT * FROM nation ", target="sqlImage", body=True, trigger="hover",
        ),
        dcc.Textarea(
            id='textarea-sql-query', placeholder='Enter your SQL query here.',
...
""" This is the Natural Language Description UI """ 
natural_language_div = html.Div(children=[
    dbc.Label("Natural Language Description"),
    html.Div([dcc.Textarea(
        id='textarea-processed-query', value='',
        style={'width': '50%', 'height': 230, 'resize': 'none'}, readOnly=True
    ),
    dcc.Textarea(
        id='textarea-natural-language', value='',
        style={'width': '50%', 'height': 230, 'resize': 'none'}, readOnly=True
    )]),
    html.Br(),
...
""" This is the right panel UI display for Natural Language Description """
body_right = html.Div([
...
app.layout = html.Div(children=[
    modal, heading, connection_details,
    body_left, spinner_boolean, error_boolean
])

def recursive_display(dict_output, div_components: list = [], title_padding=0, table_padding=1):
    """
    Procedure to recursively annotated output and display in a formatted manner

    Parameter dict_output: dictionary output from annotator

    Parameter div_components: current list of div components for displaying 

    Parameter title_padding: current title padding (for current query level)

    Parameter table_padding: current table padding (for current query level)
    """
    if (dict_output == {}):
        print("Base case")
    ...
    for i in range(len(dict_keys)):
        ...
        div_component = dash_table.DataTable(data=dict_array, 
                style_table={
                    'padding-left': table_padding, 'padding-top': 10, 'padding-bottom': 10,
                    'width': 'auto', 'max-height': '580px', 'overflowY': 'auto'
                },
...
def toggle_modal(n1, is_open):
    """
    Function to control toggling of accordion sections
    """
    if n1:
        return not is_open
    return is_open

""" Call back for SQL Query Example Modal """
app.callback(
    Output("modal-example", "is_open"), Input("example_button", "n_clicks"), State("modal-example", "is_open"),
)(toggle_modal)

""" Call back for Datasets Modal - Customer, Supplier, Nation, Region, Part, PartSupp, Orders, Lineitem """
app.callback(
...
""" Call back for Navigation Bar - About Modal """
app.callback(
    Output("modal-about", "is_open"), Input("aboutLink", "n_clicks"), State("modal-about", "is_open"),
)(toggle_modal)

""" Call back for Navigation Bar - Datasets Modal """
app.callback(
    Output("modal-datasets", "is_open"), Input("datasetsLink", "n_clicks"), State("modal-datasets", "is_open"),
)(toggle_modal)

@app.callback(
    Output('modal-database', 'is_open'), Output('port-error-message', 'is_open'), Input('dbdetails_submit_button', 'n_clicks'),
    State('host-input', 'value'), State('dbname-input', 'value'), State('username-input', 'value'),
    State('password-input', 'value'), State('port-input', 'value')
)
def get_database_inputs(n_clicks, host, dbname, username, password, port):
    """
    Callback to validate database inputs
    """
    if n_clicks:
        try:
            port = int(port)
...
@app.callback(
    Output('modal-loading', 'is_open'), Input('query_submit_button', 'n_clicks'),
    Input('textarea-sql-query', 'value'), State('loading-boolean', 'children')
)
def open_loading(n_clicks, value, loading):
    """ 
    Call back for Loading of Page when submit SQL Query 
    """
    print(ctx.inputs)
    # print(error)
...
@app.callback(
    Output('table1', 'children'), Output('loadingdiv', 'children'), Output('loading-boolean', 'children'),
    Output('error-message', 'is_open'), Output('connection-error-message', 'is_open'), Input('query_submit_button', 'n_clicks'),
    State('textarea-sql-query', 'value'), State('host-input', 'value'), State('dbname-input', 'value'),
    State('username-input', 'value'), State('password-input', 'value'), State('port-input', 'value')
)

def update_output(n_clicks, value, host, dbname, username, password, port):
    """
    Callback to update output on screen for new query
    """
    if n_clicks > 0:
        ...
        if (query_output == {}):
            ...
            return html.Div(html_output), [dbc.Modal([
                ...
                backdrop="static", id="modal-loading", size="lg", is_open=False,
            )], "false", False, False
