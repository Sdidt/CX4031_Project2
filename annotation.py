...
class Annotator:
    ...
    def find_match_in_decomposed_query(self, node: Node):
        ...
        while i < node.subquery_level:
            relevant_decomposed_query = self.decomposed_query["subqueries"]["sub_number_" + str(i + 1)]
        ...
        for k, v in relevant_info.items():
           ...
           # explanation = "The clause " + condition + " is implemented using " + node.type
            ...
            if original_query_component not in self.component_mapping:
                self.component_mapping[original_query_component] = []
            self.component_mapping[original_query_component].append(explanation)
            similarity_score = 0
            print("\n######################################################################################################################\n")   
